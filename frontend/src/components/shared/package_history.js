    ...
    if (this.props.tasks.length > 0) {
      let tasklist = this.props.tasks.map((task, index) => {
        if (task.status === "Finished") {
          return <PackageHistoryItem task={task} key={index} />;
        } else return null;
      });
      return (
        <div className="history-container">
          <h1>Completed Deliveries</h1>
          <div className="inner-history-container">{tasklist.reverse()}</div>
        </div>
      );
    } else {
      return <h1>This is where your delivery history will be located.</h1>;
    }
  }
}
